...
- Optional: GitHub import for issues as cards, basic velocity-based planning suggestions in the planner. ✅ (computeSuggestedCapacity + "Use ~N" button in +New form that fills the capacity field from avg historical done pts; dynamic placeholder + empty-state tip; +1 test; works with roundtrip CSV intervalId+status).
