...
- Optional: GitHub import for issues as cards, basic velocity-based planning suggestions in the planner. ✅ velocity done; GitHub import started (backlog "Import GitHub" button + _importFromGitHub: owner/repo + optional token, fetch open issues, map to backlog cards with simple label->type and (N) points parsing). Client fetch (public works, PAT for more).
